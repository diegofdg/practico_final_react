    ...
    try{
        ...
        if(result.length !== 0){             
            ...
            return res.status(204).json({                
                Error: 'No existen categorias registradas'
            });        
        }
    }
    catch(error){        
...
exports.categoriaUpdate = async(req,res,next) => {
    try{        
        ...
        if(!nombre || !/[a-z]+$/i.test(nombre.trim())){
            return res.status(204).json({ 'Error': 'El nombre es obligatorio, debe contener solo letras y no puede estar vacío'});
        } 
        ...
        if(result === 0) { 
            ...
            return res.status(200).json({'Mensaje': 'Categoría Actualizada.'});         
        }        
